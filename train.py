...
np.random.seed(11)
weights = 2 * np.random.rand(20000,1000) - 1
weights2 = 2 * np.random.rand(1000,22) - 1

bias = 2 * np.random.rand(1,1000) - 1
bias2 = 2 * np.random.rand(1,22) - 1
lr = 0.005
...
for j in range(epoch):
    ...
    if(labels[ri][0] == 1):
        face += 1
        current = time.time()
        print(round((current - start),1),'s',round((j/epoch*100),2),'%', np.argmax(z[0][1:])+1, np.argmax(labels[ri][1:])+1)
        if(np.argmax(z[0][1:])+1 == np.argmax(labels[ri][1:])+1):
            accurate += 1
            print("accurate : ", accurate / face * 100, "%")
